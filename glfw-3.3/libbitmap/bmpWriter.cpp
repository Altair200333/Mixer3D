...
Bitmap* BMPWriter::loadJPG(const std::string& path)
{
...
Bitmap* BMPWriter::loadBMP(const std::string& path)
{
...
bool BMPWriter::save(const Bitmap& image, std::string file_name)
{
	using namespace detail;
...
size_t BMPWriter::getPixelId(int m_width, int x, int y, int channel) const
{
	return y * m_width * 3 + y * padding(m_width) + 3 * x + channel;
}
//bytes
int BMPWriter::padding(int m_width) const
{
